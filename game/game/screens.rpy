...
screen main_menu():
    ...
    imagemap:
        ...
        #hotspot(x, y, длина, высота)


        hotspot(812, 313, 292, 57) action Start() # новая игра
        hotspot(819, 396, 286, 57) action ShowMenu("load") # загрузить
        hotspot(818, 478, 292, 57) action ShowMenu("preferences") # настройки
        hotspot(817, 562, 286, 57) action ShowMenu("about") # об игре

        hotspot(1912, 1075, 201, 153) action ShowMenu("gallery") # галерея

        hotspot(813, 646, 292, 57) action Quit(confirm=True) # выйти


screen gallery():
    tag menu
    add "images/animatios/abyss_2.png"

    grid 3 2:
        xfill True
        yfill True

        add g.make_button("room_phone", "preview1", xalign=0.5, yalign=0.5, hover_border="images/posters/frame poster1.png")
        add g.make_button("forest", "preview2", xalign=0.5, yalign=0.5, hover_border="images/posters/frame poster2.png")
        add g.make_button("three_subaru", "preview3", xalign=0.5, yalign=0.5, hover_border="images/posters/frame poster3.png")




    textbutton "Назад" text_color "#ffffff" text_hover_color "#c1e0ff" action Return() xalign 0.5 yalign 0.95
...
screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            #imagebutton xalign 0.99 yalign 3.0:
            #    idle ("gui/back.png")
            #    action Rollback()

            #    ("Назад") action Rollback()
            #    ("Пропуск") action Skip() alternate Skip(fast=True, confirm=True)
